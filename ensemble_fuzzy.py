...
if __name__ == "__main__":
    ...
    # dataset = ImageDataset(train_img, train_age)
    # data_loader = DataLoader(dataset)
    # variances = get_variance(best_model, data_loader, device)
    # print(variances)
    ...
    torch.save(best_c_model, "checkpoint/fuzzy_cl.pt")
    loss = []
    for i in range(5):
        idx = test_race == i
        print(sum(idx))
    ...
    for i in range(5):
        ...
        model = model.to('cuda')
        best_c_model = train_c(model, 30, sample_loader, val_loader)
        torch.save(best_c_model, "checkpoint/fuzzy_cl"+str(i)+".pt")
        model_list.append(best_c_model)
    ...
    for i in range(5):
        ...
        loss.append(get_performance_group_c_test(model_list, data_loader, 'cuda'))
